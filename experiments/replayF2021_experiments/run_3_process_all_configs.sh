...
PATH_PLOT_SCRIPT="module add apps/python/3.7.3; echo config \$SLURM_ARRAY_TASK_ID; python3 $SCRIPT_3_PLOT_PATHS ENAME/ c\$SLURM_ARRAY_TASK_ID"
...
for E in ${RUN[*]}; do
	...
	echo $(map $E LOG_FOLDER)/configs.csv $MIN_RAT Max : $MAX_RAT
	...
	if [ "${1,,}" == "serial" ]; then
		...
		do
			...
			echo "python3 ./scripts/log_processing/processConfig.py $(map $E LOG_FOLDER) c$c_id $(map $E SAMPLE_RATE)"
			python3 ./scripts/log_processing/processConfig.py $(map $E LOG_FOLDER) c$c_id $(map $E SAMPLE_RATE)
